        ...
        <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="col-span-2">
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Judul (Nama Proker / Nama Lomba)</label>
                    <input type="text" name="judul" value="{{ old('judul', $konten->judul) }}" required
                           class="w-full bg-gray-50 border-none py-4 px-6 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-amber-500 transition-all text-[#0a362d]">
                </div>

                {{-- KATEGORI --}}
                <div class="relative">
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Kategori</label>
                    <select name="kategori" required class="w-full bg-gray-50 border-none py-4 px-6 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-amber-500 appearance-none cursor-pointer text-[#0a362d]">
                        <option value="Proker" {{ $konten->kategori == 'Proker' ? 'selected' : '' }}>PROGRAM KERJA</option>
                        <option value="Prestasi" {{ $konten->kategori == 'Prestasi' ? 'selected' : '' }}>PRESTASI</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="file" name="gambar" class="w-full bg-gray-50 border-none py-3 px-6 rounded-2xl text-[10px] font-bold file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-[#0a362d] file:text-white">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="isi" rows="6" required class="w-full bg-gray-50 border-none py-4 px-6 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-amber-500 text-[#0a362d]">{{ old('isi', $konten->isi) }}</textarea>
                </div>
            </div>
